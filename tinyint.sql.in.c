...
CREATE DOMAIN int1 AS tinyint;

CREATE FUNCTION tinyint_to_smallint(tinyint) RETURNS smallint AS
'MODULE_PATHNAME'
LANGUAGE c IMMUTABLE STRICT;
CREATE CAST (tinyint AS smallint)
WITH FUNCTION tinyint_to_smallint(tinyint) AS IMPLICIT;

CREATE FUNCTION smallint_to_tinyint(smallint) RETURNS tinyint AS
'MODULE_PATHNAME'
LANGUAGE c IMMUTABLE STRICT;
CREATE CAST (smallint AS tinyint)
WITH FUNCTION smallint_to_tinyint(smallint) AS ASSIGNMENT;

CREATE FUNCTION tinyint_to_int(tinyint) RETURNS integer AS
'MODULE_PATHNAME'
LANGUAGE c IMMUTABLE STRICT;
CREATE CAST (tinyint AS integer)
WITH FUNCTION tinyint_to_int(tinyint) AS IMPLICIT;

CREATE FUNCTION int_to_tinyint(integer) RETURNS tinyint AS
'MODULE_PATHNAME'
LANGUAGE c IMMUTABLE STRICT;
CREATE CAST (integer AS tinyint)
WITH FUNCTION int_to_tinyint(integer) AS ASSIGNMENT;

CREATE FUNCTION tinyint_to_bigint(tinyint) RETURNS bigint AS
'MODULE_PATHNAME'
LANGUAGE c IMMUTABLE STRICT;
CREATE CAST (tinyint AS bigint)
WITH FUNCTION tinyint_to_bigint(tinyint) AS IMPLICIT;

CREATE FUNCTION bigint_to_tinyint(bigint) RETURNS tinyint AS
'MODULE_PATHNAME'
LANGUAGE c IMMUTABLE STRICT;
CREATE CAST (bigint AS tinyint)
WITH FUNCTION bigint_to_tinyint(bigint) AS ASSIGNMENT;
